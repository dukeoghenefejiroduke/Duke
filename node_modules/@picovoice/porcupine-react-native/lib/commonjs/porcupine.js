...
//
const RCTPorcupine = _reactNative.NativeModules.PvPorcupine;
class Porcupine {
  /**
   ...
   */
  static async fromBuiltInKeywords(accessKey, keywords) {
    let modelPath = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
    let device = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
    let sensitivities = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
    try {
      let {
        handle,
        frameLength,
        sampleRate,
        version
      } = await RCTPorcupine.fromBuiltInKeywords(accessKey, modelPath, device, Object.values(keywords), sensitivities);
   ...
   * @returns An instance of the engine.
   */
  static async fromKeywordPaths(accessKey, keywordPaths) {
    let modelPath = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
    let device = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
    let sensitivities = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
    try {
      let {
        ...
        sampleRate,
        version
      } = await RCTPorcupine.fromKeywordPaths(accessKey, modelPath, device, keywordPaths, sensitivities);
      return new Porcupine(handle, frameLength, sampleRate, version);
    } catch (err) {
      if (err instanceof PorcupineErrors.PorcupineError) {
        throw err;
  ...
  async process(frame) {
    if (frame === undefined) {
      throw new PorcupineErrors.PorcupineInvalidArgumentError('Frame array provided to process() is undefined or null');
    } else if (frame.length !== this._frameLength) {
      throw new PorcupineErrors.PorcupineInvalidArgumentError(`Size of frame array provided to 'process' (${frame.length}) does not match the engine 'frameLength' (${this._frameLength})`);
    }

    // sample the first frame to check for non-integer values
    if (!Number.isInteger(frame[0])) {
      throw new PorcupineErrors.PorcupineInvalidArgumentError(`Non-integer frame values provided to process(): ${frame[0]}. Porcupine requires 16-bit integers`);
    }
    try {
...
var _default = exports.default = Porcupine;
